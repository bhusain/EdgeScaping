...
if __name__ == "__main__":
    ...
    parser.add_argument('dataset', default='mnist', choices=['mnist', 'usps', 'reutersidf10k', 'pendigits', 'gene'])
    ...
    from datasets import load_mnist, load_reuters, load_usps, load_pendigits, load_gene
    ...
    elif args.dataset == 'gene':  # recommends: n_clusters=4, update_interval=20
        x = load_gene('data')
    ...
    # Define IDEC model
    idec = IDEC(dims=[x[2000:].shape[-1], 500, 500, 2000, 10], n_clusters=args.n_clusters)
    ...
    if args.ae_weights is None:
        idec.pretrain(x[0:2000], batch_size=args.batch_size, epochs=args.pretrain_epochs, optimizer=optimizer)

    # begin clustering, time not include pretraining part.

    idec.compile(loss=['kld', 'mse'], loss_weights=[args.gamma, 1], optimizer=optimizer)
    y_pred = idec.fit(x[2000:], batch_size=args.batch_size, tol=args.tol, maxiter=args.maxiter,
             update_interval=args.update_interval, ae_weights=args.ae_weights, save_dir=args.save_dir)

    print(y_pred)
    np.savetxt('gene_cluster1.txt', int(y_pred))
